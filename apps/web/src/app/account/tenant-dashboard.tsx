                ...
                {saved.slice(0, 3).map(({ property }) => {
                  ...
                  return (
                    <article key={property.id}>
                      ...
                      <div><h3><Link href={`/properties/${property.id}`}>{property.title}</Link></h3><p><MapPin aria-hidden="true" /> {property.area}, {property.city}</p><strong>{formatMaloti(property.monthlyPrice)} <small>/ month</small></strong></div>
                    </article>
                  );
                })}
              </div>
